...
def RunISHACDE(func):
    ...
    for i in range(TrialRuns):
        ...
        Best_Solution.append(BestPop)
        Best_Fitness.append(min(FitPop))
    np.savetxt("./ISHACDE_Data/GTOPX/Solution/" + str(FuncNum) + ".csv", Best_Solution, delimiter=",")
    np.savetxt("./ISHACDE_Data/GTOPX/Fitness/" + str(FuncNum) + ".csv", Best_Fitness, delimiter=",")
    np.savetxt("./ISHACDE_Data/GTOPX/Trial/" + str(FuncNum) + ".csv", All_Trial_Best, delimiter=",")


def main():
    global FuncNum, DimSize, Pop, LB, UB
    Info = [Cassini1_info, Cassini2_info, Messenger1_info, Messenger2_info, GTOC1_info, Rosetta_info, Sagas_info]
    Obj = [Cassini1, Cassini2, Messenger1, Messenger2, GTOC1, Rosetta, Sagas]
    Prob = ["Cassini1", "Cassini2", "Messenger1", "Messenger2", "GTOC1", "Rosetta", "Sagas"]

    for i in range(len(Prob)):
        DimSize, LB, UB, xbest = Info[i]()
        Pop = np.zeros((PopSize, DimSize))
        FuncNum = Prob[i]
        RunISHACDE(Obj[i])
...
if __name__ == "__main__":
    if os.path.exists('ISHACDE_Data/GTOPX/Trial') == False:
        os.makedirs('ISHACDE_Data/GTOPX/Trial')
    if os.path.exists('ISHACDE_Data/GTOPX/Solution') == False:
        os.makedirs('ISHACDE_Data/GTOPX/Solution')
    if os.path.exists('ISHACDE_Data/GTOPX/Fitness') == False:
        os.makedirs('ISHACDE_Data/GTOPX/Fitness')
    main()
