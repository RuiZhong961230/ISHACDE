...
def RunISHACDE(func):
    ...
    np.savetxt("./ISHACDE_Data/CEC2022/" + str(DimSize) + "D/F" + str(FuncNum) + ".csv", All_Trial_Best, delimiter=",")
...
def main(Dim):
    ...
    for i in range(len(CEC2022)):
        FuncNum = i + 1
        RunISHACDE(CEC2022[i].evaluate)


if __name__ == "__main__":
    if os.path.exists('ISHACDE_Data/CEC2022/10D') == False:
        os.makedirs('ISHACDE_Data/CEC2022/10D')
    if os.path.exists('ISHACDE_Data/CEC2022/20D') == False:
        os.makedirs('ISHACDE_Data/CEC2022/20D')
    for dim in [10, 20]:
        main(dim)
